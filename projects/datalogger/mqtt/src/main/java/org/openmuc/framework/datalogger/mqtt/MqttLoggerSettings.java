...
public class MqttLoggerSettings extends GenericSettings {
        ...
        public MqttLoggerSettings() {
                ...
                properties.put(CLIENT_ID, new ServiceProperty(CLIENT_ID, "MQTT client identifier", null, false));
                properties.put(PARSER,
                                new ServiceProperty(PARSER, "identifier of needed parser implementation", "openmuc",
                                                true));
                properties.put(WEB_SOCKET,
                                new ServiceProperty(WEB_SOCKET, "usage of WebSocket true/false", "false", true));

                // properties for recovery / file buffering
                properties.put(CONNECTION_RETRY_INTERVAL,
                                new ServiceProperty(CONNECTION_RETRY_INTERVAL, "connection retry interval in s", "10",
                                                true));
                properties.put(CONNECTION_ALIVE_INTERVAL,
                                new ServiceProperty(CONNECTION_ALIVE_INTERVAL, "connection alive interval in s", "10",
                                                true));
                ...
                properties.put(MULTIPLE, new ServiceProperty(MULTIPLE,
                                "if true compose log records of different channels to one mqtt message", "false",
                                true));
                properties.put(MAX_FILE_COUNT,
                                new ServiceProperty(MAX_FILE_COUNT, "file buffering: number of files to be created",
                                                "2", true));
                ...
                properties.put(MAX_BUFFER_SIZE,
                                new ServiceProperty(MAX_BUFFER_SIZE, "file buffering: buffer size in kB", "1000",
                                                true));
                ...
                properties.put(LAST_WILL_TOPIC,
                                new ServiceProperty(LAST_WILL_TOPIC, "topic on which lastWillPayload will be published",
                                                "", false));
                ...
                properties.put(FIRST_WILL_TOPIC,
                                new ServiceProperty(FIRST_WILL_TOPIC,
                                                "topic on which firstWillPayload will be published", "", false));
                properties.put(FIRST_WILL_PAYLOAD,
                                new ServiceProperty(FIRST_WILL_PAYLOAD, "payload which will be published after connect",
                                                "", false));
