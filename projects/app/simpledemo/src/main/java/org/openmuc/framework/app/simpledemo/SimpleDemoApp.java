...
public final class SimpleDemoApp {
	...
	private int setSoCEngine(int stringNumber) {
		...
		for (int i = 0; i < stringNumber; i++) {
			...
			try {
				...
				if (Vnominal > 0.0) {
					// logger.info("SETSOCENGINE: Vnominal is 2.0V for string {}. Using default
					// Vcutoff 1.75V and Vfloat 2.4V", i+1);
					vCutoff = Vnominal - 0.2;
					vFloat = Vnominal + 0.25;
				}
				if (vCutoff < 0 || vFloat < 0) {
					// logger.warn("Vcutoff or Vfloat is not set properly yet for string {}.
					// Skipping set SoC engine this cycle.", i+1);
					continue;
				}
				...
				// logger.warn("There is no value yet with this channel at string {}: {},
				// skipping set SoC engine this cycle.", i+1, e.getMessage());
				return 0;
			}
		}
		return 1;
	...
	private void calculateSoCSoH() {
		logger.info("Calculate SoC and SoH");
		for (int i = 0; i < stringNumber_1; i++) {
			for (int j = 0; j < cellNumber; j++) {
				final int si = i; // final copies for capture
				final int sj = j;
				// try{
				if (channels[si][sj][1] == null ||
						channels[si][sj][2] == null ||
						channels[si][sj][0] == null) {
					logger.warn("----------------- MODBUS: There is no channel yet at string {} cell {} --------------",
							si + 1, sj + 1);
					// channels = initiateChannel();
					continue;
				}
				if (channels[si][sj][1].getLatestRecord().getValue() == null ||
						channels[si][sj][2].getLatestRecord().getValue() == null ||
						channels[si][sj][0].getLatestRecord().getValue() == null) {
					logger.warn(
							"----------------- MODBUS: There is no value yet with this channel at string {} cell {} --------------",
							si + 1, sj + 1);
					continue;
				}

				// double current = channels[si][sj][3].getLatestRecord().getValue().asDouble()
				// / 100;
				// logger.info("Value of of {}: -----> {}",channels[si][sj][3].getId(),
				// current);
				...
				// logger.info("OLD value of voltage of string {}_cell{}: -----> {}",si+1, sj+1,
				// powerCells[si][sj].getVoltage());
				// logger.info("NEW value of voltage of string {}_cell{}: -----> {}",si+1, sj+1,
				// voltage);
				// if((voltage - powerCells[si][sj].getVoltage()) > 0) {
				// current *= -1;
				// }
				double resistance = channels[si][sj][0].getLatestRecord().getValue().asDouble() / 10000;
				// logger.info("Value of of {}: -----> {}",channels[si][sj][0].getId(),
				// resistance);

				// powerCells[si][sj].setCurrent(current);
				logger.info("String {}_Cell{}: -----> R: {}, V:{}, T: {}", si + 1, sj + 1, resistance, voltage, temp);
				// powerCells[si][sj].setVoltage(voltage);
				// logger.info("V of string {}_cell{}: -----> {}", si + 1, sj + 1, voltage);
				// powerCells[si][sj].setTemp(temp);
				// logger.info("T of string {}_cell{}: -----> {}", si + 1, sj + 1, temp);
				...
				// DoubleValue doubleSoCValue = new
				// DoubleValue(Double.parseDouble(DF.format(powerCells[si][sj].getSoc())));
				...
				// DoubleValue doubleSoHValue = new
				// DoubleValue(Double.parseDouble(DF.format(powerCells[si][sj].getSoh())));
	...
	private void initUpdateTimer() {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				// updateLatestSaveChannelNames(stringIds);
				// if (LatestValuesRestorer.restoreAll(dataAccessService,
				// latestSaveChannelNames) > 0) {
				// isRestored = true;
				// }
				// }
				// // if (isRestored) {
				...
				// logger.info("Cell dimensions are ready. Initializing power cells and
				// channels.");
				...
				// }
				// if(isUpdatedOverall){
				// if(LatestValuesRestorer.restoreAll(dataAccessService, latestSaveChannelNames)
				// > 0) {
	...
	private void getCellDimension() {
		...
		if (stringNumber != index) {
			// logger.info("String number changed from {} to {}, re-initializing cell
			// dimensions.", stringNumber, index);
		}
		if (!isFirstInitAllDimension) {
			...
			RecordListener listener = listeners.computeIfAbsent(channelID, id -> (record -> {
				if (record.getValue() != null) {
					...
					if ((channelID.startsWith("str") && channelID.contains("_cell_qty"))
							|| channelID.equals("dev_serial_comm_number")
							|| (channelID.startsWith("str") && channelID.contains("_Cnominal"))
							|| (channelID.startsWith("str") && channelID.contains("_Vnominal"))) {
