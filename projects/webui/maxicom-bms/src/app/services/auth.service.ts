...
import { OpenMucRecord } from '../services/openmuc.service'
...
export class AuthService {
  private readonly BASE_URL = '/rest'; // OpenMUC REST API
  private readonly LATEST_VALUE_API = '/rest/latest-value'; // OpenMUC REST API
    ...
    return this.http.get<LatestValueAccountResponse>(
      ...
      switchMap((response) => {
        ...
        return this.http.put(
          `${this.BASE_URL}/channels/account_1_password/latestRecord`,
          updatePayload,
          { responseType: 'text' }
        ).pipe(
          map(() => {
          ...
          catchError((err) => {
            this.showMessage('Error when changing password (latest-value API):', err.error);
            // Fallback to old OpenMUC channels API
      ...
      catchError((err) => {
        this.showMessage('Error when verifying old password (latest-value API):', err.error);
        // Fallback to old OpenMUC channels API
        return this.changePasswordFallback(oldPass, newPass);
      })
    );
